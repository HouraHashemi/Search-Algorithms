...
class butterFinderIDS:
	...
	# read file and creade data from it
	...
	# find goal by "list of forbidden char type" and "root"
	def find_goal(self,root,goal,forbidden):
		...
		while ((len(already_expanded)+len(block_list))<(int(self.dimantion[0])*int(self.dimantion[1])) ):
			...
			# main DFS
			# print("edge_list:{} root:{}".format(edge_list,root))
			path = g.DFS(root)
			if(path!=[]):
				path = path[::-1]
				break
...
if __name__ == "__main__":
	...
	ids = butterFinderIDS("input//test1.txt")
	print(np.array(ids.data))
	print("___________________________________\n\n")

	tot = []

	for b in range(len(ids.info['b'])):
		ids.print_map()
		print("====================================")
		robot  = ids.info['r'][0]
		butter = ids.info['b'][b]
		...
		robot_to_butter_path = ids.find_goal(robot,'b',['x','p'])
		print("ROBOT TO BUTTER PATH: {}".format(robot_to_butter_path))
		print("------------------------------------")

		butter_to_person_path = ids.find_goal(butter,'p',['x','b'])
		print("BUTTER TO PERSON PATH: {}".format(butter_to_person_path))
		print("------------------------------------")

		tot = ids.move_path(robot_to_butter_path,butter_to_person_path)
		...
		if tot[0]==(-1,-1):
			...
			dp = ids.direction_path(tot)
			print("DIRECTIONAL PATH: {}".format(dp))
			print("TOTAL COST: {}".format(ids.cost))
			print("DEPTH: {}".format(ids.depth))
			print("🔔 ] THE PATH IS FOUND.START? [Y/N]")
			run = input("🔅 ] ")
			if run == "y" or run == "Y":
				os.system('cls' if os.name == 'nt' else 'clear')
				ids.play_motion(dp,tot)

				# clear cell that robot was there
				ids.grid[robot_to_butter_path[0]] = ids.grid[robot_to_butter_path[0]].split('r')[0]
				# move robot to current destination cell 
				ids.grid[tot[-1]] = ids.grid[tot[-1]]+'r'
				ids.info['r'][0] = tot[-1]

				# clear cell that butter was there
				ids.grid[butter_to_person_path[0]] = ids.grid[butter_to_person_path[0]].split('b')[0]
				# move butter to current destination cell 
				ids.grid[butter_to_person_path[-1]]=ids.grid[butter_to_person_path[-1]]+'b'
				ids.info['b'][b] = butter_to_person_path[-1]
			else:
				print("[OK]")		
		print("___________________________________\n\n")
